...
public class Server {
    ...
    private void handleListRooms(Message msg) {
        ...
        rooms.forEach((room, userList) -> {
            sb.append("• ").append(room)
              .append(" (").append(userList.size()).append(userList.size() == 1 ? " user" : " users").append(")\n");
        });
        ...
        if (roomUserList != null && roomUserList.containsKey(msg.sender)) {
            Message res = new Message(Message.Type.TEXT, "Server", msg.room, sb.toString());
